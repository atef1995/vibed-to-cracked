...
export default function BlogPageClient({
  ...
  const [featuredPosts, setFeaturedPosts] =
    useState<BlogPost[]>(initialFeatured);
  ...
  // Refresh categories on client (skip if we have initial data)
  useEffect(() => {
  ...
  // Refresh featured posts on client (skip if we have initial data)
  useEffect(() => {
  ...
  const { data: postsData, isLoading } = useQuery({
    ...
    initialData:
      !debouncedSearch && !filters.category && page === 1
        ? {
            success: true,
            data: initialPosts,
            pagination: {
