...
export default function FriendsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [friendsResponse, feedResponse, notificationsResponse] =
          await Promise.all([
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {friends.map((friend) => {
                  ...
                  const friendLastActive = friend.lastActive
                    ? new Date(friend.lastActive)
                    : new Date();
            ...
            <div className="space-y-4">
                      ...
                      <div className="flex-1">
                          ...
                          <div className="mt-3 flex flex-wrap gap-3">
                            ...
                            {update.data.score && (
                              <span className="inline-flex items-center gap-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 px-2 py-1 rounded-full text-xs">
                                ...
                                {update.data.score.toPrecision(4)}% score
                              </span>
                            )}
                            {update.data.mood && (
                              <span className="inline-flex items-center gap-1 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 px-2 py-1 rounded-full text-xs">
                                {
                                  moodEmojis[
                                    update.data.mood as keyof typeof moodEmojis
                                  ]
                                }
                        ...
                        <div className="mt-4 flex items-center justify-between">
                            ...
                            {reactionTypes.map((reaction) => {
                              ...
                              return (
                                <button
                                  key={reaction.type}
                                  onClick={() =>
                                    handleReaction(update.id, reaction.type)
                                  }
                          ...
                          {update._count?.reactions &&
                            update._count.reactions > 0 && (
                              <span className="text-xs text-gray-500 dark:text-gray-400">
                                {update._count.reactions} reaction
                                {update._count.reactions !== 1 ? "s" : ""}
                              </span>
                            )}
                        </div>
                      </div>
              ...
              {progressFeed.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 dark:text-gray-400">
                    Add some friends to see their learning progress and
                    achievements!
                  </p>
                </div>
              )}
