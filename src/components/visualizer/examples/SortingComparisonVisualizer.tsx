...
import { generateBubbleSortSteps, generateSelectionSortSteps } from "../utils/algorithmSteps";
...
export function SortingComparisonVisualizer({
  ...
  const bubbleSteps = useMemo(() => generateBubbleSortSteps(initialArray), [initialArray]);
  const selectionSteps = useMemo(() => generateSelectionSortSteps(initialArray), [initialArray]);
  ...
  const bubbleTotalOps = (bubbleMetrics?.comparisons || 0) + (bubbleMetrics?.swaps || 0);
  const selectionTotalOps = (selectionMetrics?.comparisons || 0) + (selectionMetrics?.swaps || 0);

  const winner = bubbleTotalOps < selectionTotalOps ? "bubble" :
                 selectionTotalOps < bubbleTotalOps ? "selection" :
                 "tie";
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-gradient-to-r from-purple-900/30 to-blue-900/30 rounded-lg p-6 border border-purple-500/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Bubble Sort Stats */}
          <div className={`bg-black/40 rounded-lg p-4 border ${winner === "bubble" ? "border-green-500 ring-2 ring-green-500/50" : "border-purple-500/30"}`}>
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-lg font-semibold text-purple-300">Bubble Sort</h4>
              {winner === "bubble" && (
            ...
            <div className="space-y-1 text-sm text-gray-300">
              <div className="flex justify-between">
                ...
                <span className="font-mono text-yellow-400">{bubbleMetrics?.comparisons || 0}</span>
              </div>
              <div className="flex justify-between">
                <span>Swaps:</span>
                <span className="font-mono text-red-400">{bubbleMetrics?.swaps || 0}</span>
              </div>
              <div className="flex justify-between border-t border-gray-600 pt-1 mt-2">
                <span className="font-semibold">Total Operations:</span>
                <span className="font-mono font-bold text-white">{bubbleTotalOps}</span>
              </div>
            </div>
          </div>
          ...
          <div className={`bg-black/40 rounded-lg p-4 border ${winner === "selection" ? "border-green-500 ring-2 ring-green-500/50" : "border-purple-500/30"}`}>
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-lg font-semibold text-blue-300">Selection Sort</h4>
              {winner === "selection" && (
                <span className="text-xs bg-green-500 text-white px-2 py-1 rounded-full font-bold">
                  WINNER
                </span>
            ...
            <div className="space-y-1 text-sm text-gray-300">
              <div className="flex justify-between">
                <span>Comparisons:</span>
                <span className="font-mono text-yellow-400">{selectionMetrics?.comparisons || 0}</span>
              </div>
              <div className="flex justify-between">
                <span>Swaps:</span>
                <span className="font-mono text-red-400">{selectionMetrics?.swaps || 0}</span>
              </div>
              <div className="flex justify-between border-t border-gray-600 pt-1 mt-2">
                <span className="font-semibold">Total Operations:</span>
                <span className="font-mono font-bold text-white">{selectionTotalOps}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {winner === "tie" && (
          <div className="mt-4 text-center">
            <p className="text-yellow-400 font-semibold">
              It's a tie! Both algorithms performed the same number of operations.
            </p>
          </div>
        )}
        ...
        <div className="mt-4 p-3 bg-blue-900/20 border border-blue-500/30 rounded">
          <p className="text-sm text-blue-200">
            <strong>💡 Notice:</strong> Selection Sort typically makes fewer swaps than Bubble Sort,
            but may make more comparisons. The "winner" depends on the specific input data!
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-purple-900/20 rounded-lg p-4 border border-purple-500/30">
          <h4 className="text-xl font-bold text-purple-300 mb-4">Bubble Sort</h4>
          ...
          <div className="mt-4 p-3 bg-purple-900/30 rounded text-sm text-gray-300">
            ...
            <p>Repeatedly compares adjacent elements and swaps them if they're in wrong order.</p>
          </div>
        </div>

        {/* Selection Sort Panel */}
        <div className="bg-blue-900/20 rounded-lg p-4 border border-blue-500/30">
          <h4 className="text-xl font-bold text-blue-300 mb-4">Selection Sort</h4>
          ...
          <div className="mt-4 p-3 bg-blue-900/30 rounded text-sm text-gray-300">
            ...
            <p>Finds the minimum element and places it at the beginning of unsorted portion.</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div className="space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-300">
              <li>Compares <strong>adjacent</strong> elements</li>
              <li>Swaps happen <strong>frequently</strong></li>
          ...
          <div className="space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-300">
              <li>Finds <strong>minimum</strong> element each pass</li>
              <li>Swaps happen <strong>once per pass</strong></li>
