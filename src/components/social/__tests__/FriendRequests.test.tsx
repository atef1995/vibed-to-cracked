...
describe("FriendRequests - Real Null Safety Tests", () => {
  ...
  test("CRITICAL: handles undefined sender without crashing", () => {
    ...
    expect(() => {
      render(<FriendRequests requests={dangerousRequests} {...mockProps} />);
    }).not.toThrow();

    // Should show fallback content
    expect(screen.getByText("Unknown User")).toBeInTheDocument();
    expect(screen.getByAltText("Unknown User avatar")).toHaveAttribute(
      "src",
      "/default-avatar.png"
    );
  });
  ...
  test("CRITICAL: handles null image property without crashing", () => {
    ...
    expect(() => {
      render(
        <FriendRequests requests={requestsWithNullImage} {...mockProps} />
      );
    }).not.toThrow();

    expect(screen.getByText("Test User")).toBeInTheDocument();
    expect(screen.getByAltText("Test User avatar")).toHaveAttribute(
      "src",
      "/default-avatar.png"
    );
  });
  ...
  test("CRITICAL: handles completely missing sender object", () => {
    ...
    expect(() => {
      render(
        <FriendRequests requests={requestsWithMissingSender} {...mockProps} />
      );
    }).not.toThrow();
  ...
  test("Regression test: normal data still works correctly", () => {
    ...
    render(<FriendRequests requests={normalRequests} {...mockProps} />);
