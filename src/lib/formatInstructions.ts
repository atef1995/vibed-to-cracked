...
export function formatInstructions(text: string): string {
  // Extract backtick-wrapped code first so the tag/quote regexes don't
  // destroy the <code> elements we insert.
  const slots: string[] = [];
  ...
  // Style single-quoted values (violet badges)
  result = result.replace(
    /&#39;([^&#]+?)&#39;|'([^']+?)'/g,
    (_m, escaped, raw) => {
  ...
  // Restore backtick slots
  return result.replace(/\x00(\d+)\x00/g, (_m, i) => slots[parseInt(i)]);
}
