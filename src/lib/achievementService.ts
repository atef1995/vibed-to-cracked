...
interface AchievementCheck {
  userId: string;
  action: "QUIZ_COMPLETED" | "CHALLENGE_COMPLETED" | "TUTORIAL_STARTED" | "STREAK_UPDATED" | "POINTS_EARNED";
  metadata?: {
    score?: number;
    timeSpent?: number;
    streak?: number;
...
export class AchievementService {
   ...
   */
  static async checkAndUnlockAchievements({ userId, action, metadata = {} }: AchievementCheck) {
    try {
      const unlockedAchievements: (UserAchievement & { achievement: Achievement })[] = [];
      ...
      const achievementChecks = [
        ...
        {
          key: "FIRST_QUIZ",
          condition: action === "QUIZ_COMPLETED" && userStats.quizzesCompleted === 1,
          points: 10
        },
        {
          key: "FIRST_CHALLENGE", 
          condition: action === "CHALLENGE_COMPLETED" && userStats.challengesCompleted === 1,
          points: 15
        },
        ...
        {
          key: "QUIZ_MASTER_10",
          condition: action === "QUIZ_COMPLETED" && userStats.quizzesCompleted >= 10,
          points: 50
        },
        {
          key: "QUIZ_MASTER_25",
          condition: action === "QUIZ_COMPLETED" && userStats.quizzesCompleted >= 25,
          points: 100
        },
        {
          key: "PERFECT_SCORE",
          condition: action === "QUIZ_COMPLETED" && (metadata.score || 0) >= 100,
          points: 25
        },
        {
          key: "SPEED_DEMON",
          condition: action === "QUIZ_COMPLETED" && (metadata.timeSpent || 0) <= 30,
          points: 30
        },
        ...
        // Challenge Achievements
        {
          ...
          condition: action === "CHALLENGE_COMPLETED" && userStats.challengesCompleted >= 5,
          points: 40
        },
        {
          key: "CHALLENGE_MASTER_15",
          condition: action === "CHALLENGE_COMPLETED" && userStats.challengesCompleted >= 15,
          points: 75
        },
        ...
        {
          key: "CHILL_LEARNER",
          condition: action === "QUIZ_COMPLETED" && metadata.mood === "CHILL" && userStats.chillQuizzes >= 10,
          points: 30
        },
        {
          key: "FOCUSED_LEARNER", 
          condition: action === "QUIZ_COMPLETED" && metadata.mood === "FOCUSED" && userStats.focusedQuizzes >= 10,
          points: 30
        },
        {
          key: "CRACKED_LEARNER",
          condition: action === "QUIZ_COMPLETED" && metadata.mood === "CRACKED" && userStats.crackedQuizzes >= 10,
          points: 50
        }
      ];
   ...
   */
  private static async shareAchievementUnlock(userId: string, achievement: Achievement) {
    try {
