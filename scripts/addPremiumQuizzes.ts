...
async function addPremiumQuizzes() {
  ...
  try {
    ...
    if (existingQuizzes.length >= 2) {
      ...
      console.log(`✅ Updated "${existingQuizzes[1].title}" to PREMIUM`);
    }
    ...
    if (existingQuizzes.length >= 3) {
      ...
      console.log(`✅ Updated "${existingQuizzes[2].title}" to PRO`);
    }
      ...
      data: {
        ...
        questions: [
          ...
          },
          {
            ...
            correct: 1,
            explanation: 'IIFE stands for Immediately Invoked Function Expression - a function that is executed immediately after it is defined.',
            difficulty: 'easy'
          },
          {
            id: 3,
            question: 'What will this code output? (function(x) { return function(y) { return x + y; }; })(5)(3)',
            options: ['8', '53', 'undefined', 'Error'],
            correct: 0,
            explanation: 'This is a closure that returns a function. The outer function takes 5 as x, returns a function that takes 3 as y, and returns x + y = 5 + 3 = 8.',
            difficulty: 'hard'
          },
          {
            id: 4,
            question: 'Which method can be used to change the context (this) of a function?',
            options: ['call()', 'apply()', 'bind()', 'All of the above'],
            correct: 3,
            explanation: 'All three methods (call, apply, and bind) can be used to change the context (this) of a function, but they work slightly differently.',
            difficulty: 'medium'
          }
        ]
      }
        ...
        questions: [
          {
            ...
            correct: 1,
            explanation: 'Promise.all() waits for all promises to resolve (or any to reject), while Promise.race() resolves as soon as the first promise resolves or rejects.',
            difficulty: 'medium'
          },
          {
            id: 2,
            ...
            correct: 1,
            explanation: 'When an async function throws an error, it automatically returns a rejected Promise with that error.',
            difficulty: 'medium'
          },
          {
            id: 3,
            ...
            correct: 2,
            explanation: 'The Event Loop is responsible for handling asynchronous operations, managing the callback queue, and determining when callbacks should be executed.',
            difficulty: 'hard'
          },
          {
            id: 4,
            question: 'Which is the correct way to handle multiple async operations that don\'t depend on each other?',
            options: [
            ...
            correct: 3,
            explanation: 'Both approaches work, but Promise.all() is more efficient as it runs operations concurrently, while sequential await runs them one after another.',
            difficulty: 'hard'
          }
        ]
      }
    ...
    console.log(`✅ Created PRO quiz: "${proQuiz.title}"`);

    console.log('\n✅ Premium quizzes setup completed!');
    console.log('📊 Summary:');
    console.log('- Updated existing quizzes with premium tiers');
    console.log('- Created 1 PREMIUM quiz (Advanced Functions)');
    console.log('- Created 1 PRO quiz (Async JavaScript)');
