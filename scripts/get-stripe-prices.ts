...
async function getAllPrices() {
  try {
    ...
    const premiumMonthly = prices.data.find(p => {
      ...
      return product.name?.toLowerCase().includes('vibed') && 
    ...
    const proMonthly = prices.data.find(p => {
      const product = p.product as Stripe.Product;
      return product.name?.toLowerCase().includes('cracked') && 
             p.recurring?.interval === 'month' &&
             p.unit_amount === 1999; // $19.99
    });
    ...
    if (premiumMonthly) {
      console.log(`STRIPE_PREMIUM_PRICE_ID=${premiumMonthly.id}`);
    } else {
      console.log('STRIPE_PREMIUM_PRICE_ID=price_your_premium_monthly_price_id');
    }

    if (premiumAnnual) {
      console.log(`STRIPE_PREMIUM_ANNUAL_PRICE_ID=${premiumAnnual.id}`);
    } else {
      console.log('STRIPE_PREMIUM_ANNUAL_PRICE_ID=price_your_premium_annual_price_id');
    }

    if (proMonthly) {
      console.log(`STRIPE_PRO_PRICE_ID=${proMonthly.id}`);
    } else {
      console.log('STRIPE_PRO_PRICE_ID=price_your_pro_monthly_price_id');
    }

    if (proAnnual) {
      console.log(`STRIPE_PRO_ANNUAL_PRICE_ID=${proAnnual.id}`);
    } else {
      console.log('STRIPE_PRO_ANNUAL_PRICE_ID=price_your_pro_annual_price_id');
    }

    console.log('\n💡 Copy the price IDs above and update your .env.local file');
    console.log('   Make sure to use the price IDs (starting with "price_"), not product IDs!');

  } catch (error) {
    console.error('❌ Error fetching Stripe data:', error);
    console.log('\n🔧 Common solutions:');
    console.log('1. Check that your STRIPE_SECRET_KEY is correct in .env.local');
